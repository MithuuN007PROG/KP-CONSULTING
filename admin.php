...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_bike'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
     ...
     if ($edit_mode) {
        $stmt = $conn->prepare("UPDATE bikes SET name=?, model=?, specs=?, price=?, image=?, status=? WHERE id=?");
        $stmt->bind_param("sssdssi", $name, $model, $specs, $price, $image, $status, $_POST['id']);
    } else {
        $stmt = $conn->prepare("INSERT INTO bikes (name, model, specs, price, image, status) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssdss", $name, $model, $specs, $price, $image, $status);
    }
...
if (!empty($search)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM bikes WHERE (model LIKE ? OR name LIKE ?) AND status='Available'");
...
<body>
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data">
            <?php if ($edit_mode): ?>
                <?php if (isset($_GET['edit_feedback'])): ?>
...
<label>Model:</label>
<input type="text" name="model" value="<?= $edit_mode ? htmlspecialchars($current_bike['model']) : '' ?>" required>
    ...
    <input type="text" name="search" placeholder="Search by model..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
